...
# SMAP WebSocket Server - Build and Push Script
# Usage: ./build-websocket.sh [build-push|login|help]
...
show_help() {
    cat << EOF
${GREEN}SMAP WebSocket Server - Build and Push Script${NC}
...
Note:
    This builds the WebSocket server for real-time messaging.
    The server listens on port 8081 and uses Redis Pub/Sub.
