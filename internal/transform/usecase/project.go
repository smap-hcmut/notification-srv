...
func (t *projectTransformer) Transform(ctx context.Context, payload string, projectID, userID string) (*transform.ProjectNotificationMessage, error) {
	startTime := time.Now()
	...
	// Parse input message
	var inputMsg transform.ProjectInputMessage
...
func (t *projectTransformer) transformToOutput(input *transform.ProjectInputMessage) (*transform.ProjectNotificationMessage, error) {
	// Create output message
	output := &transform.ProjectNotificationMessage{
		Status: transform.ProjectStatus(input.Status),
	}
...
func (t *projectTransformer) transformProgress(input *transform.ProgressInput) (*transform.Progress, error) {
...
func (t *projectTransformer) transformProgressSafe(input *transform.ProgressInput) (*transform.Progress, error) {
	// Validate input first
	if input.Current < 0 {
...
func (t *projectTransformer) TransformPhaseBased(ctx context.Context, payload string, projectID, userID string) (*transform.ProjectPhaseNotificationMessage, error) {
	startTime := time.Now()

	// Parse input message
	var inputMsg transform.ProjectPhaseInputMessage
	if err := json.Unmarshal([]byte(payload), &inputMsg); err != nil {
...
func (t *projectTransformer) transformPhaseToOutput(input *transform.ProjectPhaseInputMessage) *transform.ProjectPhaseNotificationMessage {
	output := &transform.ProjectPhaseNotificationMessage{
		Type: input.Type,
		Payload: transform.ProjectPhasePayloadOutput{
...
func (t *projectTransformer) transformPhaseProgress(input *transform.PhaseProgressInput) *transform.PhaseProgress {
	...
	return &transform.PhaseProgress{
		Total:           input.Total,
		Done:            input.Done,
		Errors:          input.Errors,
...
// Returns either *transform.ProjectNotificationMessage or *transform.ProjectPhaseNotificationMessage
