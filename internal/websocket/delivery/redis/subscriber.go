...
import (
	...
	transformUC "notification-srv/internal/transform/usecase"
...
func NewSubscriber(client pkgRedis.IRedis, hub *usecase.Hub, logger log.Logger) *Subscriber {
	ctx, cancel := context.WithCancel(context.Background())

	// Create transform layer dependencies
	validator := transformUC.NewInputValidator()
	metrics := transformUC.NewMetricsCollector()
	errorHandler := transformUC.NewErrorHandler(logger, metrics)
	transformer := transformUC.NewMessageTransformer(validator, metrics, errorHandler, logger)
...
func (s *Subscriber) Start() error {
	...
	s.pubsub = s.client.GetClient().PSubscribe(s.ctx, s.patternChannels...)
